...
void Stack_Init( Stack *stack ) {
	...
	stack->array = (char*)malloc(sizeof(stack->array)*STACK_SIZE);
	if (NULL == stack->array)
	{
		Stack_Error(SERR_INIT);
		free(stack);
...
void Stack_Pop( Stack *stack ) {
	...
	// Overeni zda-li neni nahodou zasobnik prazdny
	if (0 != Stack_IsEmpty(stack))
	{
		return;
	}
	...
	{
		// Snizeni vrcholu o jeden Charakter 
		stack->topIndex--;
	}
...
void Stack_Push( Stack *stack, char data ) {
	...
	// Kontrola zda-li neni nahodou plny zasobnik 
	if (0 != Stack_IsFull(stack))
	{
